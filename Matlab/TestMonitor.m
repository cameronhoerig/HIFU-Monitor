...
funcAmp = 2;%.170; % start input at 170 mVpp
...
while(1)
    if(sampleFlag == 1)
        currentTime = tic;
        fprintf(picObject,'g'); % tell the dsPIC to start sampling data
        sampleFlag = 0;
        pause(.5-toc(currentTime));
    else
        currentTime = tic;
        fprintf(picObject,'s'); % tell the dsPIC to stop sampling data
        sampleFlag = 1;
        pause(.5-toc(currentTime));
    end
end
