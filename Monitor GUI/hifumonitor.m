...
set(handles.pauseButton,'UserData',callbackData);
...
set(handles.transducerStatus,'String','Transducer Off');
set(handles.inputVoltage,'String','Input Voltage: Not On');
...
picObject.InputBufferSize = 10000;
picObject.Timeout = 1;
comObject{1} = picObject;
set(handles.startButton,'UserData',comObject);
...
if(pauseOperation == 0)
    ...
    picObject.BytesAvailableFcnMode = 'byte';
    picObject.BytesAvailableFcnCount = 1;
    picObject.BytesAvailableFcn = {@SerialCallback,handles};
    fopen(picObject);
end
pauseOperation = 0;
set(handles.stopButton,'UserData',pauseOperation);
graphHandle = handles.pauseButton;
...
axes(handles.RmsPlot);
plot(dataToPlot);
fwrite(picObject,'g'); % start the dsPIC sampling


function SerialCallback(obj, event,handles)
...
callbackData = get(handles.pauseButton,'UserData');
dataPlotLength = callbackData(1);
dataToPlot = callbackData(2:dataPlotLength+1);
dataIndex = callbackData(end);
